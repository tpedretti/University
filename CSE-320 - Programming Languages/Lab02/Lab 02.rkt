...
(define (partEven x y) (integer? (/ (+ x y) 2)))
(define curriedEve (curry partEven))
(curriedEve)
((curriedEve) 10)
(((curriedEve) 10) 56)

(define (makeString x y) (string-append x y))
(define createString (curry makeString))
(createString)
((createString) "Hello")
(((createString) "Hello") " World")

(define (randomNum  min max) (random min max))
(define getRanNum (curry randomNum))
(getRanNum)
((getRanNum) 1)
(((getRanNum) 1) 100)

(define (newAdd x y) (+ x y))
(define curriedAdd (curry newAdd))
(curriedAdd)
((curriedAdd) 5)
(((curriedAdd) 5) 5621)
...
(define (addTwice f x) (f (f x)))
(define (add n) (+ n n))
(addTwice add 2)

(define (strings f x) (f (f x)))
(define (newString n) (string-append n n))
(strings newString "Test ")

(define (newRanNum f x) (random (f x)))
(define (newRan n) (random n))
(newRanNum newRan 54)

(define (isNewInt f x) (integer? (f x)))
(define (newInt n) (integer? n))
(isNewInt newInt 3)

;Functions that return Functions
(define (make-adder1 x) (lambda (y) (+ x y)))
(define two-adder (make-adder1 2))

(define (HAHARANDOM x) (random x))
(define harandom (HAHARANDOM 5))

(define (connectString x) (string-append x))
(define newNewString (connectString "Stringg"))

(define (isMyString x) (string? x))
(define testString (isMyString "test"))

(define ((idkBFFJILL x) z) (lambda (y) (* x (+ x z y) z)))
(define BFF ((idkBFFJILL 2)2))
